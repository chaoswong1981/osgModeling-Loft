...
Loft::Loft( const Loft& copy, const vEngine::CopyOp& copyop/*=vEngine::CopyOp::SHALLOW_COPY*/ ):
...
{
    const vEngine::Vec3Array* pts = path->getPath();
    ...
    if ( from!=shapes.end() && to==shapes.end() )
    {
        ...
        for ( itr=from+1; itr!=shapes.end(); ++itr )
            *itr = new Curve( *fromSect, vEngine::CopyOp::DEEP_COPY_ALL );
    }
    return true;
}

bool Loft::buildTransitions( Loft::Shapes::iterator& from, Loft::Shapes::iterator& to, const vEngine::Vec3* pathPtr )
{
    Curve* fromSect = dynamic_cast<Curve*>( (*from).get() );
    Curve* toSect = dynamic_cast<Curve*>( (*to).get() );
    vEngine::Vec3Array* fromArray = dynamic_cast<vEngine::Vec3Array*>( fromSect->getPath() );
    vEngine::Vec3Array* toArray = dynamic_cast<vEngine::Vec3Array*>( toSect->getPath() );
    int numToGenerate = to-from-1;
    if ( numToGenerate<=0 ) return false;

    // Get length of the path between 2 exist shapes..
    double maxLen = 0.0f;
    const vEngine::Vec3* currPtr, *lastPtr=pathPtr;
    ...
    for ( Loft::Shapes::iterator itr=from+1; itr!=to; ++itr )
    {
        ...
        *itr = new Curve( *fromSect, vEngine::CopyOp::DEEP_COPY_ALL );
        vEngine::Vec3Array* currArray = dynamic_cast<vEngine::Vec3Array*>( (*itr)->getPath() );

        vEngine::Vec3Array::iterator vitr=currArray->begin();
        for ( unsigned int i=0;
            vitr!=currArray->end();
            ++vitr, ++i )
        {
            vEngine::Vec3 baseVec = (*toArray)[i] - (*fromArray)[i];
...
vEngine::Vec3 Loft::considerBasisX( const vEngine::Vec3 basisZ )
{
    vEngine::Vec3 basisX;
    if ( basisZ.z()!=0.0f )
    {
        if ( vEngine::equivalent(basisZ.x(),0.0f) && vEngine::equivalent(basisZ.y(),0.0f) )
    ...
    {
        if ( vEngine::equivalent(basisZ.x(),0.0f) && vEngine::equivalent(basisZ.z(),0.0f) )
    ...
    {
        if ( vEngine::equivalent(basisZ.y(),0.0f) && vEngine::equivalent(basisZ.z(),0.0f) )
...
void Loft::updateImplementation()
{
    ...
    {
        vEngine::notify(vEngine::WARN) << "osgModeling: Loft object should have a profile with at least 2 points." << std::endl;
        return;
    }
    if ( !_shapes.size() )
    {
        vEngine::notify(vEngine::WARN) << "osgModeling: Loft object should have at least 1 section." << std::endl;
        return;
    }
    ...
    {
        vEngine::notify(vEngine::WARN) << "osgModeling: Loft object has " << _shapes.size() << " sections now."
            "But only " << _profile->getPath()->size() << " may be accepted by the profile." << std::endl;
    }

    // Initiate vertics & texture coordinates.
    vEngine::ref_ptr<vEngine::Vec3Array> vertics = new vEngine::Vec3Array;
    vEngine::ref_ptr<vEngine::Vec2Array> texCoords = new vEngine::Vec2Array;

    // Build vertex array.
    vEngine::ref_ptr<vEngine::Vec3Array> lastIntersects = new vEngine::Vec3Array;
    vEngine::Vec3Array* pts = _profile->getPath(), *lastArray=NULL;
        ...
        ++sitr, ++i )
    {
		...
		if (i!=0 && i!=knots-1)
		{
			vEngine::Vec3 tmp = (*pts)[i+1] - (*pts)[i];
			tmp.normalize();
        ...
        vEngine::Vec3Array* array = dynamic_cast<vEngine::Vec3Array*>( curve->getPath() );
        shapeSize = array->size();
        lastIntersects->resize( shapeSize );
        for ( vEngine::Vec3Array::iterator vitr=array->begin();
            vitr!=array->end();
            ++vitr )
        {
            unsigned int pos = vitr - array->begin();
            vEngine::Vec3 ip, v = newZ;
            if ( lastArray )
            {
                ...
                v += ( (*lastArray)[pos] - (*vitr) ) * 
                    vEngine::Matrix::rotate( vEngine::Vec3(0.0f,0.0f,1.0f), newZ );
                ip = calcIntersect( (*lastIntersects)[pos], v, vEngine::Plane(newZ, (*pts)[i]) );
            }
            else
            {
                ...
                vEngine::Vec3 newX = considerBasisX( newZ );
                vEngine::Matrix transMat = coordSystemMatrix( (*pts)[i],
                    newX, vEngine::Vec3(0.0f,0.0f,0.0f), newZ );
                ip = (*vitr) * transMat;
            }
    ...
    vEngine::Vec3 topCenter, botCenter;
    vEngine::BoundingBox topBox, botBox;
    ...
    GLenum bodyType = vEngine::PrimitiveSet::QUAD_STRIP;
    GLenum capType = vEngine::PrimitiveSet::TRIANGLE_FAN;
    if ( getAuxFunctions()&Model::USE_WIREFRAME )
    {
        bodyType = vEngine::PrimitiveSet::LINES;
        capType = vEngine::PrimitiveSet::LINE_STRIP;
    }
    ...
    if ( getGenerateParts()&Model::BODY_PART )
    {
        for ( i=0; i<knots-1; ++i )
        {
            vEngine::ref_ptr<vEngine::DrawElementsUInt> bodySeg = new vEngine::DrawElementsUInt( bodyType, 0 );
    ...
    {
        vEngine::ref_ptr<vEngine::DrawElementsUInt> cap1 = new vEngine::DrawElementsUInt( capType, 0 );
    ...
    if ( getGenerateParts()&Model::CAP2_PART && shapeSize>2 )
    {
        ...
        vEngine::ref_ptr<vEngine::DrawElementsUInt> cap2 = new vEngine::DrawElementsUInt( capType, 0 );
    ...
    vEngine::Vec3Array::iterator itr;
    if ( getGenerateCoords()&Model::TEX_COORDS )
    {
		...
		vEngine::Vec3Array* array = _shapes[0]->getPath();
		double step = (array->at(0) - array->at(1)).length() * shapeSize;
        ...
        for ( itr=pts->begin(); itr!=pts->end(); ++itr )
        {
            ...
            for ( j=0; j<shapeSize; ++j )
                texCoords->push_back( vEngine::Vec2(j*interval, currLen / step/*currLen*maxTexCoordOfBody/maxLen*/) );
        }

        // Coords of cap1 is limited in (0.0, 0.5) - (0.5, 1.0), cap2 in (0.5, 0.5) - (1.0, 1.0).
        vEngine::Vec2 mapPoint;
        vEngine::BoundingBox newRect;
        if ( getGenerateParts()&Model::CAP1_PART )
        {
            newRect.set( vEngine::Vec3(0.0f,0.5f,0.0f), vEngine::Vec3(0.5f,1.0f,0.0f) );
        ...
        {
            newRect.set( vEngine::Vec3(0.5f,0.5f,0.0f), vEngine::Vec3(1.0f,1.0f,0.0f) );
