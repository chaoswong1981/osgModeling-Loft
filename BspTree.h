...
/** BSP tree class.
 */
class BspTree : public vEngine::Object
{
public:
    struct BspFace;

    typedef std::vector<vEngine::Vec3> PointList;
    typedef std::vector<BspFace> FaceList;
    ...
    struct BspFace
    {
        ...
        BspFace() {}
        bool addPoint( vEngine::Vec3 p, bool replaceSame=true );
        bool insertPoint( PointList::iterator pos, vEngine::Vec3 p, bool ingoreSame=true );
        inline bool valid() { return _points.size()>2; }
        inline double orientation( vEngine::Vec3 refNormal );
        inline void reverse();
        inline vEngine::Vec3 operator[] ( unsigned int i ) { return _points[i]; }
        vEngine::BoundingBox getBound();
    };

    struct BspNode
    {
        vEngine::Plane _plane;
        FaceList _coinFaces;
        BspNode* _posChild;
        BspNode* _negChild;

        BspNode( vEngine::Plane p ):
            _plane(p), _posChild(0), _negChild(0)
        {}
    };

    BspTree( unsigned int numSearchBestDivider=5 );
    BspTree( const BspTree& copy, const vEngine::CopyOp& copyop=vEngine::CopyOp::SHALLOW_COPY );
    ...
    inline vEngine::BoundingBox getBound() { return _bound; }
    ...
    */
    static FaceClassify partitionFace( vEngine::Plane plane, BspFace face, BspFace& posFace, BspFace& negFace );
